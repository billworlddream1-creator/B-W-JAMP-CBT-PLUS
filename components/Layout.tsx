...
const Layout: React.FC<LayoutProps> = ({ children, userTier, theme, onNavigateHome, onNavigatePricing, onNavigateAdmin }) => {
  ...
  return (
    <div className={`min-h-screen flex flex-col bg-${theme.bg} transition-colors duration-[2000ms] pb-24 lg:pb-0`}>
      ...
      <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-md lg:hidden z-[100] bg-white/80 backdrop-blur-xl border border-slate-200 rounded-[2.5rem] shadow-2xl p-2 flex justify-around items-center">
        {navItems.map((item) => (
          <button 
            key={item.label}
            onClick={item.onClick}
        ...
        <a 
          ...
          className="flex flex-col items-center justify-center py-3 px-6 rounded-[2rem] hover:bg-emerald-50 transition-all group"
        >
          ...
          <span className="text-[8px] font-black uppercase mt-1 tracking-widest text-emerald-500">Support</span>
        </a>
      </nav>
